...
for i = 0:tSlice - 1
    ...
    if(i > 0)
        CPD{i * 6 + 1} = eye(2);
        CPD{i * 6 + 2} = [
            epsilon/2 , 1-epsilon , epsilon/2 , 0;
            0 , epsilon/2 , 1-epsilon , epsilon/2;
            epsilon/2 , 0 , epsilon/2 , 1-epsilon;
            1-epsilon , epsilon/2 , 0 , epsilon/2];
    end
...
BEL = CreateFactorGraph(parentList,childrenList,nValues,prior,CPD,EL,64);
